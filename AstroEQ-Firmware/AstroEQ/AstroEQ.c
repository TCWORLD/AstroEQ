...
bool decodeCommand(char command, char* buffer){ //each command is axis specific. The axis being modified can be retrieved by calling synta_axis()
    ...
    switch(command) {
        ...
        case 'I': //set slew speed, return empty response (this sets the speed to move at if in slew mode)
            ...
            break;
        case 'M': //Go-To midpoint is unused. But ensure it is a recognized command.
        ...
        case 'O': //Control GPIO1 or Programming Mode
            if (axis == RA) {
                if (progMode != RUNMODE) {
                    //:O command for RA in programming mode configures output mode for snap pin 
                    snapPinIsOpenDrain = buffer[0] - '0';
                } else {
                    //:O commands to the RA axis in run mode control GPIO1 (SNAP2 port)
                    if (snapPinIsOpenDrain) {
